                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php if(isset($user->getprofiles->address1)): ?>
                                                    <?php echo $user->getprofiles->address1; ?>

                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <?php if($user->status == 0): ?> <font color="green" class="btn btn-info btn-sm">Active</font> <?php elseif($user->status == 1): ?>
                                                    <font color="red"
                                                          class="btn btn-danger btn-sm">Deactivate</font> <?php else: ?> <font
                                                    color="gray" class="btn btn-warning btn-sm">Pending</font> <?php endif; ?>
                                            </td>
                                            <td class="text-center">
                                                <?php if(auth()->check() && auth()->user()->hasRole('super')): ?>
                                                <a href="<?php echo route('users.show',$user); ?>" title="รายละเอียด"
                                                   class="btn btn-info btn-sm"><i class="fa fa-eye"></i></a>
                                                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('ALL-Plivilege')): ?>
                                                    <?php if(isset($user->getprofiles->address1)): ?>
                                                        <a title="แก้ไข" href="<?php echo route('users.edit',$user); ?>"
                                                           class="btn btn-success btn-sm"><i
                                                                class="fa fa-pencil"></i></a>
                                                    <?php endif; ?>
                                                    <a title="ลบ" onclick="deleteRow('<?php echo $user->id; ?>')"
                                                       class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></a>
                                                <?php endif; ?>
                                                <?php endif; ?>
                                                <form method="POST" class="hidden" id="formDelete<?php echo $user->id; ?>"
                                                      action="<?php echo route('users.destroy',$user); ?>">
                                                    <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\HeroCar\resources\views/users/index.blade.php ENDPATH**/ ?>